...
static bool check_dim_layer(struct mtk_drm_crtc *mtk_crtc)
{
	int i = 0;
	bool exist = 0;
...
static int mtk_drm_idlemgr_monitor_thread(void *data)
{
	...
	while (1) {
		...
		/* When have dim layer in vdo mode, do not enter idle */
		if (mtk_crtc_is_frame_trigger_mode(crtc) == 0 &&
			check_dim_layer(mtk_crtc)) {
			DDP_MUTEX_UNLOCK(&mtk_crtc->lock, __func__,
				__LINE__);
			continue;
		}
...
{
	struct mtk_drm_crtc *mtk_crtc = to_mtk_crtc(crtc);
	struct mtk_ddp_comp *comp_ovl0 = mtk_ddp_comp_find_by_id(crtc, DDP_COMPONENT_OVL0);
	struct mtk_ddp_comp *comp_ovl0_2l = mtk_ddp_comp_find_by_id(crtc, DDP_COMPONENT_OVL0_2L);
	...
	DDP_MUTEX_LOCK(&mtk_crtc->lock, __func__, __LINE__);
	if (!comp_ovl0->funcs || !comp_ovl0->funcs->io_cmd) {
		DDPINFO("cannot find output component\n");
		return -1;
	}
	comp_ovl0->funcs->io_cmd(comp_ovl0, NULL, OVL_BG_COLOR, &bg_color);

	if (!comp_ovl0_2l->funcs || !comp_ovl0_2l->funcs->io_cmd) {
		DDPINFO("cannot find output component\n");
		return -1;
	}
	comp_ovl0_2l->funcs->io_cmd(comp_ovl0_2l, NULL, OVL_BG_COLOR, &bg_color);
...
static int mtk_drm_idle_for_meta_mode(void *data)
{
	static long count = 0;
    ...
    while (1) {
        ...
        _primary_path_idle_for_meta_mode(crtc,0x00);
        msleep(30000);
        count ++;
        if(count % 2) {
			_primary_path_idle_for_meta_mode(crtc,0x01);
        } else {
			_primary_path_idle_for_meta_mode(crtc,0x02);
        }
    }
...
*/
    if (get_boot_mode() == META_BOOT) {
        idlemgr->idle_for_meta_mode=kthread_create(mtk_drm_idle_for_meta_mode,crtc,"disp_idle_meta_mode");
        wake_up_process(idlemgr->idle_for_meta_mode);
    }
#endif
