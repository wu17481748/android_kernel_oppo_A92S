...
*/
    if (get_boot_mode() == META_BOOT) {
        if (meta_mode_set_once == 0) {
            ...
            cmdq_pkt_write(handle, comp->cmdq_base,
                       comp->regs_pa + DISP_REG_OVL_ROI_BGCLR, 0xFF00FF00,
                       ~0);
...
static void mtk_ovl_mask(struct mtk_ddp_comp *comp, u32 offset, u32 mask, u32 data)
{
	u32 temp = readl(comp->regs + offset);

	writel((temp & ~mask) | (data & mask), comp->regs + offset);
}

static void mtk_ovl_set_roi_bgcolor(struct mtk_ddp_comp *comp, unsigned int bg_color)
{
	struct mtk_drm_crtc *mtk_crtc = comp->mtk_crtc;
	u32 width, height;
	u32 tmp_reg1;

	width = mtk_crtc->avail_modes->hdisplay;
	height = mtk_crtc->avail_modes->vdisplay;
	DDPDBG("%s, width = 0x%x,height = 0x%x,\n", __func__, width, height);
	if (width != 0 && height != 0) {
		mtk_ovl_mask(comp, DISP_REG_OVL_ROI_SIZE, 0xfffffff, height << 16 | width);
		_store_bg_roi(comp, height, width);
	}

	tmp_reg1 = readl(comp->regs + DISP_REG_OVL_SRC_CON);

	if (comp->id == DDP_COMPONENT_OVL0)
		mtk_ovl_mask(comp, DISP_REG_OVL_SRC_CON, 0xffffffff, tmp_reg1 & 0xfffffff0);
	else if (comp->id == DDP_COMPONENT_OVL0_2L)
		mtk_ovl_mask(comp, DISP_REG_OVL_SRC_CON, 0xffffffff, tmp_reg1 & 0xfffffffc);

	mtk_ovl_mask(comp, DISP_REG_OVL_ROI_BGCLR, 0xffffffff, bg_color);
			  ...
			  enum mtk_ddp_io_cmd io_cmd, void *params)
{
	...
	case OVL_BG_COLOR:
	{
		u32 bg_color = *(unsigned int *)params;

		mtk_ovl_set_roi_bgcolor(comp, bg_color);
		break;
	}
	case IRQ_LEVEL_ALL: {
		unsigned int inten;
