		...
		unsigned int ddp_mode)
{
	...
	if (cur_hrt_bw != NO_PENDING_HRT &&
		...
		DDPINFO("cur:%u last:%u, release HRT to last_hrt_req\n",
			cur_hrt_bw,	mtk_crtc->qos_ctx->last_hrt_req);
...
#if 0
	/*resume mode flush frame data,reset Doze_EN_END_done,DISP_ON_done,flush_num*/
	if(crtc->state->active && (state->prop_val[CRTC_PROP_DOZE_ACTIVE] == 0)){
		DOZE_EN_END_done = 0;
		flush_num = 0;
	}

	/*resume to doze mode flush frame data more than 3 frame,set DOZE_EN_END CMD*/
	if(!DOZE_EN_END_done && crtc->state->active && state->prop_val[CRTC_PROP_DOZE_ACTIVE]){
		if(flush_num == 3){
			DDPINFO("%s:DOZE EN END +\n", __func__);
			mtk_drm_crtc_set_panel_disp(crtc,DOZE_EN_END);
			DOZE_EN_END_done = 1;
		}
	}
	/*doze mode flush frame data every time,flush_num++*/
	if(crtc->state->active && state->prop_val[CRTC_PROP_DOZE_ACTIVE]){
		flush_num++;
	}
#endif
	for_each_comp_in_cur_crtc_path(comp, mtk_crtc, i, j) {
