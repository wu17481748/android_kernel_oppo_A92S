...
#ifdef VENDOR_EDIT
/*Feiping.Li@Camera.Driver 20190925, add to control flashlight via proc file*/
                                                                     ...
                                                                     loff_t *data)
{
	...
	if(regBuf[0] == '0') {
		...
		mutex_unlock(&fl_mutex);
		if (ret != 0) {
			pr_err("Failed to disable flash.\n");
		}
...
static int flashlight_probe(struct platform_device *dev)
{
	...
	#endif
	#ifdef VENDOR_EDIT
	/*Feiping.Li@Camera.driver, 20190925  add for flashlight contorl via proc */
	flash_proc_init();
	#endif /*VENDOR_EDIT*/
