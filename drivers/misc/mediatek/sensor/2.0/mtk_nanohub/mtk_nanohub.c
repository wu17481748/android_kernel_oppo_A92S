...
#include <linux/suspend.h>
//#ifdef VENDOR_EDIT
/* laq@PSW.BSP.sensor,2020/01/13, add for 19040 */
...
//#ifdef VENDOR_EDIT
//zhouxu@PSW.BSP.sensor,2019/11/21, add utc print
...
static void mtk_nanohub_init_sensor_info(void)
{
	...
	//ifdef VENDOR_EDIT
	//tangjh@PSW.BSP.sensor,2019/11/06, add sensor
	p = &sensor_state[SENSOR_TYPE_LUX_AOD];
	p->sensorType = SENSOR_TYPE_LUX_AOD;
	p->rate = SENSOR_RATE_ONCHANGE;
	p->gain = 1;
	strlcpy(p->name, "luxaod", sizeof(p->name));
	...
	p->rate = SENSOR_RATE_ONCHANGE;
	p->gain = 1;
	strlcpy(p->name, "freefall", sizeof(p->name));
	strlcpy(p->vendor, "oppo", sizeof(p->vendor));
	
	p = &sensor_state[SENSOR_TYPE_PICKUP_MOTION];
	p->sensorType = SENSOR_TYPE_PICKUP_MOTION;
	p->rate = SENSOR_RATE_ONCHANGE;
	p->gain = 1;
	strlcpy(p->name, "pkup", sizeof(p->name));
	strlcpy(p->vendor, "oppo", sizeof(p->vendor));
	
	p = &sensor_state[SENSOR_TYPE_ACTION_DETECT];
	p->sensorType = SENSOR_TYPE_ACTION_DETECT;
	p->rate = SENSOR_RATE_ONCHANGE;
	p->gain = 1;
	strlcpy(p->name, "amd", sizeof(p->name));
	strlcpy(p->vendor, "oppo", sizeof(p->vendor));
	
	p = &sensor_state[SENSOR_TYPE_FFD];
	p->sensorType = SENSOR_TYPE_FFD;
	p->rate = SENSOR_RATE_ONCHANGE;
	p->gain = 1;
	strlcpy(p->name, "ffd", sizeof(p->name));
	strlcpy(p->vendor, "oppo", sizeof(p->vendor));
	//endif 
//#ifdef VENDOR_EDIT
...
static int sensor_list_proc_show(struct seq_file *m, void *v)
{
    ...
    pr_err("sensor_list proc show start\n");
    seq_puts(m, "dynamic hardware sensorlist:\n");
    for (handle = accel_handle; handle < maxhandle; handle++) {
        type = sensorlist_handle_to_sensor(handle);
        if (type < 0)
            continue;
        p = &sensor_state[type];
        seq_printf(m, "sensortype:%d vendor:%d chipname:%s\n",
            type, p->vendor, p->name);
    }
    return 0;
}
...
static int mtk_nanohub_report_to_manager(struct data_unit_t *data)
{
	...
	if (data->flush_action == DATA_ACTION) {
		...
		case ID_PROXIMITY:
			...
			pr_notice("%s,[%d,%d,%d]\n", __func__, event.sensor_type,event.word[0],event.word[1]);
			break;
		case ID_PRESSURE:
			event.timestamp = data->time_stamp;
			event.sensor_type = id_to_type(data->sensor_type);
		...
		case ID_LUX_AOD:
			...
			event.sensor_type = id_to_type(data->sensor_type);
			event.action = data->flush_action;
			event.word[0] = data->lux_aod_event.state;
			event.word[1] = data->lux_aod_event.report_count;
		...
		case ID_FREE_FALL:
			...
			event.sensor_type = id_to_type(data->sensor_type);
			event.action = data->flush_action;
			event.word[0] = data->free_fall_event.free_fall_time;
			event.word[1] = data->free_fall_event.angle;
			event.word[2] = data->free_fall_event.report_count;
		...
		case ID_PICKUP_MOTION:
			...
			event.sensor_type = id_to_type(data->sensor_type);
			event.action = data->flush_action;
			event.word[0] = data->pickup_motion_event.value;
			event.word[1] = data->pickup_motion_event.report_count;
			pr_notice("%s [%d,%d]\n", __func__, event.sensor_type,event.word[0]);
			break;
		case ID_ACTION_DETECT:
			...
			event.sensor_type = id_to_type(data->sensor_type);
			event.action = data->flush_action;
			event.word[0] = data->action_detect_event.value;
			event.word[1] = data->action_detect_event.report_count;
			pr_notice("%s [%d,%d]\n", __func__, event.sensor_type,event.word[0]);
			break;
		case ID_FFD:
			event.timestamp = data->time_stamp;
			event.sensor_type = id_to_type(data->sensor_type);
			event.action = data->flush_action;
			event.word[0] = data->ffd_event.value;
			event.word[1] = data->ffd_event.report_count;
			pr_notice("%s [%d,%d]\n", __func__, event.sensor_type,event.word[0]);
			break;
		...
		case ID_GYROSCOPE:
			...
			break;
                //#ifdef VENDOR_EDIT
                //tangjh@PSW.BSP.Sensor 2019/11/19 add for msensor save calibration
		case ID_MAGNETIC_FIELD:
			event.timestamp = data->time_stamp;
			event.sensor_type = id_to_type(data->sensor_type);
			event.accurancy = data->magnetic_t.status;
		...
		event.word[4] = data->data[4];
		event.word[5] = data->data[5];
		pr_notice("%s ,%d,%d\n",__func__, event.sensor_type, event.action);
	}
	/*
...
//ifdef VENDOR_EDIT
//zhouxu@PSW.BSP.Sensor, 2019-12-24, add wakeup event
			data->sensor_type == ID_LUX_AOD ||
			data->sensor_type == ID_FREE_FALL ||
			data->sensor_type == ID_PICKUP_MOTION ||
			data->sensor_type == ID_ACTION_DETECT ||
			data->sensor_type == ID_FFD ||
//endif VENDOR_EDIT
	...
	// pr_notice("%s  event.timestamp = %lld, sensor_type = %d, action = %d, event.word = [%d, %d, %d]\n", 
         //       __func__, event.timestamp, event.sensor_type, event.action, event.word[0], event.word[1], event.word[2]);
			...
			void *unused)
{
	...
	case PM_POST_SUSPEND:
		...
		mtk_nanohub_send_timestamp_to_hub();
//ifdef VENDOR_EDIT
//zhouxu@PSW.BSP.Sensor, 2019-11-21, add to print UTC time in scp
...
//#ifdef VENDOR_EDIT
//zhouxu@PSW.BSP.sensor,2019/11/22, add reg rw
static uint8_t store_reg = 0;
static uint8_t store_type = 0;
static void scp_rw_reg_func(uint8_t type, uint8_t *data)
{
	union SCP_SENSOR_HUB_DATA req;
	int err = 0;
	req.set_cust_req.sensorType = type;
	req.set_cust_req.action = SENSOR_HUB_SET_CUST;
	req.set_cust_req.rwReg.action = CUST_ACTION_RW_REGISTER;
	req.set_cust_req.rwReg.buff[0]= data[0];
	req.set_cust_req.rwReg.buff[1]= data[1];
	req.set_cust_req.rwReg.buff[2]= data[2];


	err = mtk_nanohub_req_send(&req);
	if (err == 0) {
		if (req.set_cust_rsp.action != SENSOR_HUB_SET_CUST || 0 != req.set_cust_rsp.errCode) {
			pr_err("::scp_sensorHub_req_send::CUST_ACTION_RW_REGISTER::failed  1!\n");
		}
		if (req.set_cust_rsp.rwReg.action != CUST_ACTION_RW_REGISTER) {
			pr_err("::scp_sensorHub_req_send::CUST_ACTION_RW_REGISTER::failed  2!\n");
		}
		((uint8_t*)data)[0] = (uint8_t)req.set_cust_rsp.rwReg.buff[0];
	} else {
...
static ssize_t mtk_nanohub_reg_show(struct device_driver *ddri, char *buf)
{
	uint8_t reg_buff[3] = {0};

	reg_buff[0] = 0; /*0 means READ register*/
	reg_buff[1] = store_reg;
	scp_rw_reg_func(store_type, reg_buff);

	return sprintf(buf,"Reg[0x%02x] = 0x%02x[type:%02x][prox_tip:%02x]\n", store_reg, reg_buff[0], store_type, ID_PROXIMITY);
}

static ssize_t mtk_nanohub_reg_store(struct device_driver *ddri, const char *buf, size_t count)
{
	uint8_t reg_buff[3] = {0};
	unsigned int addr, val, type;
	reg_buff[0] = 1; /*1 means WRITE register*/
	sscanf(buf, "%x %x %x", &addr, &val, &type);
	reg_buff[1] = (uint8_t)addr;
	reg_buff[2] = (uint8_t)val;
	store_reg = reg_buff[1];
	store_type = (uint8_t)type;
...
//#ifdef VENDOR_EDIT
//tangjh@PSW.bsp.sensor 2019/01/13 add for debug reg
static DRIVER_ATTR_RW(mtk_nanohub_reg);
static struct driver_attribute *mtk_nanohub_attrs[] = {
	&driver_attr_trace,
	&driver_attr_state,
        &driver_attr_mtk_nanohub_reg,
...
static void scp_sync_utc_func(struct work_struct *dwork)
{
	...
	int err = 0;
	if (READ_ONCE(rtc_compensation_suspend)) {
		pr_err("sync utc rtc_compensation_suspend suspend,drop time sync\n");
		return;
	}
	...
	req.set_cust_req.syncUTC.u32_data[0] = utc_data[0];
	req.set_cust_req.syncUTC.u32_data[1] = utc_data[1];
	req.set_cust_req.syncUTC.u32_data[2] = utc_data[2];
	req.set_cust_req.syncUTC.u32_data[3] = utc_data[3];

	err = mtk_nanohub_req_send(&req);
	if (err < 0 || req.rsp.action != SENSOR_HUB_SET_CUST) {
		pr_err("%s fail!\n", __func__);
		return;
	}
	queue_delayed_work(scp_sync_utc_wq, &scp_sync_utc_dw, msecs_to_jiffies(1000));
