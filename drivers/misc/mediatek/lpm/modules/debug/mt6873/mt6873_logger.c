	...
	(struct mt6873_spm_wake_status *wakesta)
{
	...
	for (i = 0; i < IRQ_NUMBER; i++) {
		...
		if (mt6873_spm_wakesrc_irqs[i].wakesrc & wakesta->r12) {
			...
			if (mt_irq_get_pending(irq_no)) {
				log_wakeup_reason(irq_no);
				if(mt6873_spm_wakesrc_irqs[i].wakesrc == WAKE_SRC_STA1_EINT_EVENT_B) {//mt6358-irq
...
#undef LOG_BUF_SIZE
	...
	if (type == MT_LPM_ISSUER_SUSPEND) {
		...
		record_suspend_r13_info(mt6873_wake.r13, mt6873_wake.debug_flag, mt6873_wake.timer_out, plat_mmio_read(SPM_BK_VTCXO_DUR));
		#endif
...
struct r13_blocker_detail_info mt6873_wakeup_r13_table[] = {
...
int mt6873_wakeup_r13_table_size = sizeof(mt6873_wakeup_r13_table)/sizeof(mt6873_wakeup_r13_table[0]);
void record_suspend_r13_count_clear(void)
{
	...
	for(i= 0; i< mt6873_wakeup_r13_table_size; i++) {
		 mt6873_wakeup_r13_table[i].count = 0;
		 mt6873_wakeup_r13_table[i].duration = 0;
	}
}
...
void get_mt6885_wakeup_r13_table(struct mt6885_sleep_static_info **address, struct r13_blocker_detail_info **base, int **size)
{
	...
	*base = &mt6873_wakeup_r13_table[0];
	*size = &mt6873_wakeup_r13_table_size;
	return;
}
...
void record_suspend_r13_info(u32 r13, u32 debug_flag, u32 timer_out, u32 clock_26m_off)
{
	...
	for(i= 0; i< mt6873_wakeup_r13_table_size; i++) {
		if((r13 & mt6873_wakeup_r13_table[i].bitinfo) != 0) {
			mt6873_wakeup_r13_table[i].count++;
			mt6873_wakeup_r13_table[i].duration += ndeep_sleep_duration_cur;
