...
struct fpc1020_data {
        ...
        int power_on_value;   /* power_on/off_value is used to indicate the GPIO value for power pin*/
        int power_off_value;
#endif /* VENDOR_EDIT */
};
...
int fpc_power_on(struct fpc1020_data* fpc_dev)
{
    ...
    for (index = 0; index < fpc_dev->power_num; index++) {
        ...
        case FP_POWER_MODE_GPIO:
            gpio_set_value(fpc_dev->pwr_list[index].pwr_gpio, fpc_dev->power_on_value);
...
int fpc_power_off(struct fpc1020_data* fpc_dev)
{
    ...
    for (index = 0; index < fpc_dev->power_num; index++) {
        ...
        case FP_POWER_MODE_LDO:
            ...
            break;
        case FP_POWER_MODE_GPIO:
            gpio_set_value(fpc_dev->pwr_list[index].pwr_gpio, fpc_dev->power_off_value);
            pr_info("set pwr_gpio 1\n");
            break;
        case FP_POWER_MODE_AUTO:
            pr_info("[%s] power on auto, no need power on again\n", __func__);
...
#ifdef VENDOR_EDIT
#define PROJECT_USE_GPIO_POWER	(get_project() == 19131 || get_project() == 19420 || get_project() == 19421 || get_project() == 20001 || get_project() == 20002 || get_project() == 20003 || get_project() == 20200)
...
static ssize_t hardware_reset(struct device *dev, struct device_attribute *attribute, const char *buffer, size_t count)
{
        ...
        if (get_project() == 18531 || get_project() == 18161 || get_project() == 19132 || get_project() == 19133 || PROJECT_USE_GPIO_POWER)  {
            struct  fpc1020_data *fpc1020 = dev_get_drvdata(dev);
...
int fpc_parse_pwr_list(struct fpc1020_data* fpc_dev) {
    ...
    if (PROJECT_USE_GPIO_POWER) {
        fpc_dev->power_on_value = 0;
        fpc_dev->power_off_value = 1;
    } else {
        fpc_dev->power_on_value = 1;
        fpc_dev->power_off_value = 0;
    }
    ...
    for_each_available_child_of_node(np, child) {
        ...
        case FP_POWER_MODE_GPIO:
            ...
            gpio_direction_output(pwr_list[child_node_index].pwr_gpio, fpc_dev->power_off_value);
            pr_err("set fpc_pwr off %u output %d \n", child_node_index, fpc_dev->power_off_value);
            break;
...
static int fpc1020_irq_probe(struct platform_device *pldev)
{
       ...
       if (get_project() == 18531 || get_project() == 18161 || get_project() == 19132 || get_project() == 19133 ||  PROJECT_USE_GPIO_POWER) {
        ...
        if (get_project() != 18531 && get_project() != 18161 && get_project() != 19131 && get_project() != 19132 && get_project() != 19133 && get_project() != 19420 && get_project() != 19421) {
           fpc_power_on(fpc1020);
        }
        #endif /* VENDOR_EDIT */
