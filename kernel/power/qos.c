			   ...
			   s32 new_value)
{
	...
	pr_info("pm_qos_class=%d value=%d owner=%s\n", req->pm_qos_class, new_value, req->owner);
			...
			int pm_qos_class, s32 value)
{
	...
	pr_info("pm_qos_class=%d value=%d owner=%s\n", req->pm_qos_class, value, req->owner);
				   ...
				   unsigned long timeout_us)
{
	...
	pr_info("pm_qos_class=%d value=%d owner=%s timeout=%ld\n", req->pm_qos_class, new_value, req->owner, timeout_us);
	if (new_value != req->node.prio) {
		pm_qos_update_target(
			pm_qos_array[req->pm_qos_class]->constraints,
			&req->node, PM_QOS_UPDATE_REQ, new_value);
...
void pm_qos_remove_request(struct pm_qos_request *req)
{
	...
	trace_pm_qos_remove_request(req->pm_qos_class, PM_QOS_DEFAULT_VALUE,
			req->owner);
	pr_info("pm_qos_class=%s value=%d owner=%s", req->pm_qos_class, PM_QOS_DEFAULT_VALUE, req->owner);
